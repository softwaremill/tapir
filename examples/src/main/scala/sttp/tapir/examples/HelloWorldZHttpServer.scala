...
object HelloWorldZHttpServer extends App {
  ...
  val app: HttpApp[Blocking, Throwable] = ZHttpInterpreter().toRoutes(helloWorld)(name => ZIO.succeed(s"Hello $name")) <>
    ZHttpInterpreter().toRoutes(add) { case (x, y) => ZIO.succeed(s"Adding up ${x + y}") }
