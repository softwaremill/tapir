...
object HelloWorldZioHttpServer extends App {
  ...
  val app: HttpApp[Blocking, Throwable] = ZioHttpInterpreter().toRoutes(helloWorld)(name => ZIO.succeed(Right(s"Hello $name"))) <>
    ZioHttpInterpreter().toRoutes(add) { case (x, y) => ZIO.succeed(Right(s"Adding up ${x + y}")) }
