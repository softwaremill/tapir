    ...
    typeData match {
      ...
      case TypeData(s @ TSchema(st: TSchemaType.SOpenProduct, _, _, _, _, _, _, schemaValidator), codecValidator) =>
        (st.info -> TypeData(s, codecValidator): ObjectTypeData) +: apply(
          TypeData(
            st.valueSchema,
            elementValidator(schemaValidator)
          )
        )
      case _ => List.empty
    }
