...
class SwaggerUITest extends AsyncFunSuite {
  ...
  private val initOptionsList = List((None,""),(Some(initOptions1.toMap), injection1), (Some(initOptions2.toMap),injection2), (Some(initOptions3.toMap), injection3))
  private val oAuthInitOptionsList = List((None, ""), (Some(Map.empty[String,String]),oAuthStringWrap("")), (Some(oAuthInitOptions1.toMap),oAuthStringWrap(oInjection1)), (Some(oAuthInitOptions2.toMap),oAuthStringWrap(oInjection2)),(Some(oAuthInitOptions3.toMap),oAuthStringWrap(oInjection3)))
  ...
  test("SwaggerUI optionsInjection Combination Testing"){
    initOptionsList.foreach({
      (initOptions, injection) => oAuthInitOptionsList.foreach({
        (oAuthInitOptions, oInjection) =>
