...
class OpenAPIVerifier private (endpoints: List[AnyEndpoint], yamlOpenApiSpec: String, mode: Mode) {
  ...
  /** Verifies the compatibility of the Tapir endpoints with the provided OpenAPI specification.
    ...
    *
    * @return
    *   a boolean indicating whether the endpoints are compatible with the OpenAPI specification.
    *   - `true` if the endpoints are compatible.
    *   - `false` otherwise.
    */
  ...
  def verify(): Boolean = {
    ...
    mode match {
      case Mode.AtMost =>
        ...
        openAPIComparator.compare().isEmpty
      ...
      case Mode.Exact =>
        ...
        openAPIComparator.compare().isEmpty && openAPIComparatorA.compare().isEmpty
    }
  }
