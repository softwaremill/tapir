...
object SchemaAnnotationsMacroImpl {
  def derived[T: c.WeakTypeTag](c: blackbox.Context): c.Expr[SchemaAnnotations[T]] = {
    ...
    val HideInDocsAnn = typeOf[sttp.tapir.Schema.annotations.hideInDocs]
    ...
    val hideInDocs = annotations.collectFirst { case ann if ann.tree.tpe <:< HideInDocsAnn => q"""true""" }
    ...
    c.Expr[SchemaAnnotations[T]](
      q"""_root_.sttp.tapir.internal.SchemaAnnotations.apply($description, $encodedExample, $default, $format, $deprecated, $hideInDocs, $encodedName, $validator)"""
    )
  }
}
