...
/** Allows intercepting the handling of `request`, before decoding using any of the endpoints is done. The request
  ...
  * interceptors are run, logic decoding subsequent endpoint inputs will be run.
  *
  ...
  *
  * Moreover, when calling `requestHandler`, an [[EndpointInterceptor]] can be provided, which will be added to the list
  * of endpoint interceptors to call. The order in which the endpoint interceptors will be called will correspond to
...
/** Allows intercepting the handling of a request by an endpoint, when either the endpoint's inputs have been
  * decoded successfully, or when decoding has failed. Ultimately, when all interceptors are run, the endpoint's
...
trait Responder[F[_], B] {
  def apply[O](valuedEndpointOutput: ValuedEndpointOutput[O]): F[ServerResponse[B]]
}
