...
object ServerDefaults {
  ...
  def validationErrorToMessage(ve: ValidationError[_]): String = ve.validator match {
    case Validator.Min(value)           => s"expected ${ve.invalidValue} to be greater than or equal to $value"
    case Validator.Max(value)           => s"expected ${ve.invalidValue} to be less than or equal to $value"
