...
trait CodecMacros {
  /** Creates a codec for an enumeration, where the validator is derived using [[sttp.tapir.Validator.derivedEnumeration]].
    ...
    *
    * @tparam L The type of the low-level representation of the enum, typically a [[String]] or an [[Int]].
    * @tparam T The type of the enum.
    * @param decode How low-level values are decoded to the enum value. `None` if the low-level value is invalid, that
    *               is, when there's no high-level enum value.
    * @param encode How the enum value is encoded as a low-level value.
    */
