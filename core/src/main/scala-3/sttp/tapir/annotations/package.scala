...
package object annotations {
  inline def deriveEndpointInput[T <: Product]: EndpointInput[T] = ${AnnotationsMacros.deriveEndpointInputImpl[T]}

  def deriveEndpointOutput[A]: EndpointOutput[A] = ??? // TODO
}

object AnnotationsMacros {
  def deriveEndpointInputImpl[T <: Product: Type](using q: Quotes): Expr[EndpointInput[T]] = {
    import quotes.reflect.*

    // annotation symbols
    val endpointInputAnnotationSymbol = TypeTree.of[annotations.endpointInput].tpe.typeSymbol
    val pathAnnotationSymbol = TypeTree.of[annotations.path].tpe.typeSymbol
    val queryAnnotationSymbol = TypeTree.of[annotations.query].tpe.typeSymbol
    val headerAnnotationSymbol = TypeTree.of[annotations.header].tpe.typeSymbol
    val headersAnnotationSymbol = TypeTree.of[annotations.headers].tpe.typeSymbol
    val cookieAnnotationSymbol = TypeTree.of[annotations.cookie].tpe.typeSymbol
    val cookiesAnnotationSymbol = TypeTree.of[annotations.cookies].tpe.typeSymbol
    val paramsAnnotationSymbol = TypeTree.of[annotations.params].tpe.typeSymbol
    val bearerAnnotationSymbol = TypeTree.of[annotations.bearer].tpe.typeSymbol
    val basicAnnotationSymbol = TypeTree.of[annotations.basic].tpe.typeSymbol
    val apikeyAnnotationSymbol = TypeTree.of[annotations.apikey].tpe.typeSymbol
    val securitySchemeNameAnnotationSymbol = TypeTree.of[annotations.securitySchemeName].tpe.typeSymbol
    val bodyAnnotationSymbol = TypeTree.of[annotations.body].tpe.typeSymbol

    // schema symbols
    val descriptionAnnotationSymbol = TypeTree.of[description].tpe.typeSymbol
    val deprecatedAnnotationSymbol = TypeTree.of[deprecated].tpe.typeSymbol

    def summonCodec[L: Type, H: Type, CF <: CodecFormat: Type](field: CaseClassField[q.type, T]): Expr[Codec[L, H, CF]] = Expr.summon[Codec[L, H, CF]].getOrElse {
      report.throwError(s"Cannot summon codec from: ${Type.show[L]}, to: ${Type.show[H]}, formatted as: ${Type.show[CF]}, for field: ${field.name}.")
    }

    // inputs
    def makeQueryInput[f: Type](field: CaseClassField[q.type, T])(altName: Option[String]): Expr[EndpointInput.Basic[f]] = {
      val name = Expr(altName.getOrElse(field.name))
      '{query[f]($name)(using ${summonCodec[List[String], f, TextPlain](field)})}
    }

    def makeHeaderIO[f: Type](field: CaseClassField[q.type, T])(altName: Option[String]): Expr[EndpointIO.Basic[f]] = {
      val name = Expr(altName.getOrElse(field.name))
      '{header[f]($name)(using ${summonCodec[List[String], f, TextPlain](field)})}
    }

    def makeCookieInput[f: Type](field: CaseClassField[q.type, T])(altName: Option[String]): Expr[EndpointInput.Basic[f]] = {
      val name = Expr(altName.getOrElse(field.name))
      '{cookie[f]($name)(using ${summonCodec[Option[String], f, TextPlain](field)})}
    }

    def makeBodyIO[f: Type](field: CaseClassField[q.type, T])(ann: Term): Expr[EndpointIO.Basic[f]] = {
    ...
    def makeQueryParamsInput[f: Type](field: CaseClassField[q.type, T]): Expr[EndpointInput.Basic[f]] = {
      summon[Type[f]] match {
    ...
    def makeHeadersIO[f: Type](field: CaseClassField[q.type, T]): Expr[EndpointInput.Basic[f]] =
      summon[Type[f]] match {
    ...
    def makeCookiesIO[f: Type](field: CaseClassField[q.type, T]): Expr[EndpointInput.Basic[f]] =
      summon[Type[f]] match {
    ...
    def makePathInput[f: Type](field: CaseClassField[q.type, T]): Expr[EndpointInput.Basic[f]] = {
      val name = Expr(field.name)
      '{path[f]($name)(using ${summonCodec[String, f, TextPlain](field)})}
    }

    def makeFixedPath(segment: String): Expr[EndpointInput.Basic[Unit]] = '{stringToPath(${Expr(segment)})}

    // auth inputs
    def makeBearerAuthInput[f: Type](field: CaseClassField[q.type, T], schemeName: Option[Term], auth: Term): Expr[EndpointInput.Single[f]] =
    ...
    def makeBasicAuthInput[f: Type](field: CaseClassField[q.type, T], schemeName: Option[Term], auth: Term): Expr[EndpointInput.Single[f]] =
      setSecuritySchemeName(
    ...
    def wrapInput[f: Type](field: CaseClassField[q.type, T], input: Expr[EndpointInput.Single[f]]): Expr[EndpointInput.Single[f]] = {
    ...
    def wrapWithApiKey[f: Type](input: Expr[EndpointInput.Single[f]], apikey: Option[Term], schemeName: Option[Term]): Expr[EndpointInput.Single[f]] =
      apikey
        .map(ak => setSecuritySchemeName('{EndpointInput.Auth.ApiKey($input, ${ak.asExprOf[annotations.apikey]}.challenge, None)}, schemeName))
        .getOrElse(input)

    def setSecuritySchemeName[f: Type](auth: Expr[EndpointInput.Auth[f]], schemeName: Option[Term]): Expr[EndpointInput.Single[f]] =
      schemeName
        .map(s => '{$auth.securitySchemeName(${s.asExprOf[annotations.securitySchemeName]}.name)})
        .getOrElse(auth)

    def addSchemaMetadata[f: Type](field: CaseClassField[q.type, T], input: Expr[EndpointInput.Single[f]]): Expr[EndpointInput.Single[f]] = {
    ...
    def addMetadataToBasic[f: Type](field: CaseClassField[q.type, T],
    ...
    def mapToTargetFunc[A: Type, T: Type](inputIdxToFieldIdx: mutable.Map[Int, Int], caseClass: CaseClass[q.type, T]): Expr[A => T] = {
    ...
    def mapFromTargetFunc[A: Type, T: Type](inputIdxToFieldIdx: mutable.Map[Int, Int], caseClass: CaseClass[q.type, T]): Expr[T => A] = {
    ...
    result.tpe.asType match {
      case '[EndpointInput[r]] =>
        '{${result.asExprOf[EndpointInput[r]]}.map[T](${mapToTargetFunc[r, T](inputIdxToFieldIdx, caseClass)})(${mapFromTargetFunc[r, T](inputIdxToFieldIdx, caseClass)})}
    }
  }
}
