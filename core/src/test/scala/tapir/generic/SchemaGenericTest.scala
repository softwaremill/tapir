...
class SchemaTest extends FlatSpec with Matchers {
  ...
  val expectedDSchema: SProduct = SProduct(SObjectInfo("tapir.generic.D"), List(("someFieldName", stringSchema)))
  ...
  it should "find schema for recursive data structure" in {
    ...
    schema shouldBe SProduct(
      ...
      List(("f1", Schema(SRef(SObjectInfo("tapir.generic.F"))).asArrayElement), ("f2", intSchema))
    )
  }
  ...
  it should "use custom schema for custom types" in {
    ...
    schema shouldBe SProduct(SObjectInfo("tapir.generic.G"), List(("f1", intSchema), ("f2", stringSchema)))
  }

  it should "derive schema for parametrised type classes" in {
    val schema = implicitly[Schema[H[A]]].schemaType
    schema shouldBe SProduct(SObjectInfo("tapir.generic.H", List("A")), List(("data", expectedASchema)))
  }
  ...
  it should "find schema for map of products" in {
    ...
    schema shouldBe SOpenProduct(
      ...
      Schema(SProduct(SObjectInfo("tapir.generic.D"), List(("someFieldName", stringSchema))))
    )
  }
    ...
    schema shouldBe SOpenProduct(
      ...
      Schema(
        SProduct(
          ...
          List(("data", Schema(SProduct(SObjectInfo("tapir.generic.D"), List(("someFieldName", stringSchema))))))
        )
      )
    )
  }
              ...
              "source" ->
                Schema(
                  SCoproduct(
                    SObjectInfo("tapir.generic.Node", List.empty),
                    List(
                      Schema(SRef(SObjectInfo("tapir.generic.Edge"))),
                      Schema(
                        SProduct(
  ...
  it should "find the right schema for a case class with simple types" in {
    ...
    schema.schemaType shouldBe SProduct(
      SObjectInfo("tapir.generic.SchemaTest.<local SchemaTest>.Test1"),
      List(
    ...
    val expectedISchema: Schema[IOpt] =
      Schema(
        SProduct(
          ...
          List(("i1", Schema(SRef(SObjectInfo("tapir.generic.IOpt")), isOptional = true)), ("i2", intSchema))
        )
      )
    val expectedJSchema: Schema[JOpt] = Schema(SProduct(SObjectInfo("tapir.generic.JOpt"), List(("data", expectedISchema.asOptional))))
    ...
    val expectedISchema: Schema[IList] =
      Schema(
        SProduct(
          ...
          List(("i1", Schema(SRef(SObjectInfo("tapir.generic.IList"))).asArrayElement), ("i2", intSchema))
        )
      )
    val expectedJSchema =
      Schema(SProduct(SObjectInfo("tapir.generic.JList"), List(("data", expectedISchema.asArrayElement))))
