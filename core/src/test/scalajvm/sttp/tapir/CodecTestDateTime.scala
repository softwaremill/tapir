...
class CodecTestDateTime extends AnyFlatSpec with Matchers with Checkers {

  private implicit val arbitraryInstant: Arbitrary[Instant] = Arbitrary(Gen.posNum[Long].map(Instant.ofEpochMilli))

  private implicit val arbitraryDuration: Arbitrary[Duration] =
    Arbitrary(for {
  ...
  private implicit val arbitraryScalaDuration: Arbitrary[SDuration] =
    Arbitrary(arbitraryDuration.arbitrary.map(d => SDuration.fromNanos(d.toNanos): SDuration))

  private implicit val arbitraryLocalTime: Arbitrary[LocalTime] = Arbitrary(
