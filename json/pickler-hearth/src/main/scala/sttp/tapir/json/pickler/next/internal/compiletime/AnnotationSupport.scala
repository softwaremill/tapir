...
/** Cross-platform access to the annotations the tapir `Schema` derivation cares about.
  *
  * Hearth's `Type#annotations` / `Parameter#annotations` already abstract over the Scala 2 and Scala 3 reflection APIs, so nothing here is
  * platform-specific. They are lowered to [[UntypedExpr]] so that the caller can re-type them as `Any` and splice them into a `List[Any]`
  * that the runtime helpers fold over — which keeps annotation *matching* out of the macro entirely.
  */
trait AnnotationSupport { this: MacroCommons & StdExtensions =>

  /** Annotations on a constructor parameter, including those inherited from a member of the same name declared on a base class or trait.
    *
    ...
    * }}}
    * `Dog.name` must pick up `@description("name")`. Hearth's `Parameter#annotations` only reports annotations written on the parameter
    * itself, so the base classes are walked explicitly.
    *
    * An annotation declared on the parameter wins over an inherited one of the same type, which is what lets a subclass override a parent's
    * `@description`.
    */
  ...
  /** The literal argument of a field's `@encodedName`, if the field has one.
    *
    * `Left` when the annotation is present but its argument is not a string literal: the codec half needs the value during expansion (it
    * becomes a jsoniter field-name mapping), so a computed argument cannot be honoured, and silently falling back to the configured
    * transformation would let the schema (which folds the annotation at runtime) disagree with the JSON.
    */
  ...
  /** Annotations on a type, including those inherited from its base classes.
    *
    * Note that `@encodedName` is deliberately *not* consumed from this list when building an `SName` — see `SchemaDerivation.sNameExpr`. A
    * parent's `@encodedName` must not rename its subtypes.
    */
  ...
  /** The string argument of a single-string annotation, when it is a literal.
    *
    * Used for `@encodedName` on a type, which has to be known at compile time because it replaces the `SName` that other parts of the
    * derivation (notably the discriminator mapping) are built from.
    */
