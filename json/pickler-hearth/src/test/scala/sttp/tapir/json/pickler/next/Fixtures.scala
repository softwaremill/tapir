...
/** Verbatim port of the uPickle-based module's `Fixtures` (only the package differs), so that the ported suites keep their
  * fully-qualified-name assertions modulo the `next` segment.
  */
object Fixtures:
  enum ColorEnum:
    case Green, Pink
