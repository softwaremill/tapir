...
// The only edit relative to `json/pickler`'s copy of this file: the implementation under test lives in the `next`
// package (see PICKLER_HEARTH_PLAN.md §11.1). Every assertion, and every fixture FQN it depends on, is unchanged --
// the fixtures are declared in this file, so they keep their `sttp.tapir.json.pickler.*` names.
