...
class SchemaRecursionTest extends AnyFlatSpec with Matchers with OptionValues {
  ...
  it should "emit an SRef whose SName is exactly the enclosing schema's name" in {
    ...
    ref shouldBe SName("sttp.tapir.json.pickler.next.SchemaRecursionTest.RecList")
  ...
  it should "handle recursion through a sealed hierarchy" in {
    val schema = Pickler.schemaFor[Node]
    val subtypes = schema.schemaType.asInstanceOf[SCoproduct[Node]].subtypes

    subtypes.flatMap(_.name.map(_.fullName.split('.').last)) should contain theSameElementsAs List("Edge", "SimpleNode")

    val edge = subtypes.find(_.name.exists(_.fullName.endsWith("Edge"))).value
    // `Edge.source: Node` points back at the coproduct itself.
...
// depend on the enclosing-class naming rules, which is `SchemaDerivationTest`'s business, not this file's.
sealed trait Node
