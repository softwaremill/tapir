...
/** Recursive-type support for the schema half of the derivation. The expectations are modelled on tapir core's own `Schema.derived`.
  *
  ...
  * schema, because `Schema.applyValidation` resolves a reference by looking it up in a `Map[SName, Schema]` accumulated during traversal. A
  * merely *stable* name is not enough — a mismatch degrades silently to "no validation" rather than raising an error, which is why
  * `should "resolve ..."` below is the load-bearing test in this file.
  */
