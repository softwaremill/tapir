...
class CreateDerivedEnumerationPickler[T: ClassTag](
  ...
  inline def apply(
    ...
    new Pickler[T](reader, writer, schema)
  }
  ...
  private inline def readWriterForEnumerationCase[C]: (Types#Reader[C], Types#Writer[C]) =
    val reader = annotate[C](SingletonReader[C](upickleMacros.getSingleton[C]), upickleMacros.tagName[C])
    val writer = TapirPickle
      .annotate[C](SingletonWriter[C](null.asInstanceOf[C]), upickleMacros.tagName[C], Annotator.Checker.Val(upickleMacros.getSingleton[C]))
    (reader, writer)
