...
/** Import `sttp.tapir.json.pickler.generic.auto.*` for automatic pickler derivation: a [[Pickler]] will be derived at the use site for
  ...
  *
  * There is deliberately no `Mirror.Of[T]` requirement — the derivation handles primitives, collections and `Map`s as well as product and
  * sum types, so constraining it to Mirrors would exclude exactly the types that need no user-visible ceremony.
  */
object auto {
