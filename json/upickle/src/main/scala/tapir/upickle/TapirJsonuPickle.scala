...
trait TapirJsonuPickle {
  ...
  implicit def encoderDecoderCodec[T: RW, M]: JsonCodec[T] = new JsonCodec[T] {

    def encode(t: T): String = write(t)
    def decode(s: String): DecodeResult[T] = {
      Try(read[T](s)) match {
        case Success(v) => Value(v)
        case Failure(e) => Error("upickle", e)
      }
    }
  ...
  // ???
  implicit val schemaForuPickleJson: SchemaFor[String] =
    SchemaFor(
      SProduct(
        SObjectInfo("com.lihaoyi.upickle"),
        List.empty,
        List.empty
      )
    )
