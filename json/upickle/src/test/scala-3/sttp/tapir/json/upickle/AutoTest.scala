...
class AutoTest extends AnyFlatSpecLike with Matchers {
  ...
  object TestFixture:
    ...
    enum SimpleEnum derives tapirPickle.ReadWriter:
      ...
      case SimpleCaseTwo extends SimpleEnum

    sealed trait AnimalKind
    object AnimalKind {
      implicit val rw: tapirPickle.ReadWriter[AnimalKind] = tapirPickle.ReadWriter.merge(tapirPickle.macroRW[Mammal.type], tapirPickle.macroRW[Bird.type])
    }
    @upickle.implicits.key("Mammal") case object Mammal extends AnimalKind {
    }
    ...
    case class Dog(name: String, dogAge: DogAge, dogOwner: DogOwner, kind: AnimalKind, simpleEnum: SimpleEnum)
  ...
  it should "work" in {
    ...
    val dogJsonStr = tapirPickle.write(
      Dog("Bob", DogAge(33), DogOwner(name = "Anthony", ownerAge = 39), Mammal, SimpleEnum.SimpleCaseTwo
    ))
