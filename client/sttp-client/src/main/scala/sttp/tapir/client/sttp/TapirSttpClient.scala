...
trait TapirSttpClient {
  ...
  implicit class RichEndpoint[I, E, O, R](e: Endpoint[I, E, O, R]) {
    ...
    def toSttpRequestUnsafe(
        baseUri: Uri
    )(implicit wsToPipe: WebSocketToPipe[R]): I => Request[Either[E, O], R] =
      SttpClientInterpreter(sttpClientOptions).toRequestThrowDecodeFailures(e, Some(baseUri))
    ...
    def toSttpRequest(
        baseUri: Uri
    )(implicit wsToPipe: WebSocketToPipe[R]): I => Request[DecodeResult[Either[E, O]], R] =
      SttpClientInterpreter(sttpClientOptions).toRequest(e, Some(baseUri))
  }
}
