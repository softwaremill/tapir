...
case class PekkoHttpServerOptions(
  ...
  def prependInterceptor(i: Interceptor[Future]): PekkoHttpServerOptions = copy(interceptors = i :: interceptors)
  def appendInterceptor(i: Interceptor[Future]): PekkoHttpServerOptions = copy(interceptors = interceptors :+ i)
}
...
object PekkoHttpServerOptions {
  ...
  val defaultSlf4jServerLog: DefaultServerLog[Future] = {
    val log = LoggerFactory.getLogger(PekkoHttpServerInterpreter.getClass)
  ...
  def default(implicit ec: ExecutionContext): PekkoHttpServerOptions = customiseInterceptors.options
}
