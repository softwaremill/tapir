...
trait AkkaHttpServerInterpreter {
  def toDirective[I, E, O](e: Endpoint[I, E, O, AkkaStreams with WebSockets with Effect[Future]])(implicit
  ...
  def toRoute[I, E, O](e: Endpoint[I, E, O, AkkaStreams with WebSockets with Effect[Future]])(logic: I => Future[Either[E, O]])(implicit
      serverOptions: AkkaHttpServerOptions
  ): Route =
    new EndpointToAkkaServer(serverOptions).toRoute(e.serverLogic(logic))

  def toRouteRecoverErrors[I, E, O](
      e: Endpoint[I, E, O, AkkaStreams with WebSockets with Effect[Future]]
  ...
  def toDirective[I, E, O](serverEndpoint: ServerEndpoint[I, E, O, AkkaStreams with WebSockets with Effect[Future], Future])(implicit
      serverOptions: AkkaHttpServerOptions
  ): Directive[(I, Future[Either[E, O]] => Route)] =
    new EndpointToAkkaServer(serverOptions).toDirective(serverEndpoint.endpoint)

  def toRoute[I, E, O](serverEndpoint: ServerEndpoint[I, E, O, AkkaStreams with WebSockets with Effect[Future], Future])(implicit
      serverOptions: AkkaHttpServerOptions
  ): Route = new EndpointToAkkaServer(serverOptions).toRoute(serverEndpoint)

  //

  def toRoute(serverEndpoints: List[ServerEndpoint[_, _, _, AkkaStreams with WebSockets with Effect[Future], Future]])(implicit
      serverOptions: AkkaHttpServerOptions
  ): Route = {
