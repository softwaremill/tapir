...
class Http4sTestServerInterpreter extends TestServerInterpreter[IO, Fs2Streams[IO] with WebSockets, Http4sServerOptions[IO], Routes] {
  ...
  private val serverBuilder = EmberServerBuilder
    ...
    // Keep ember's default idle timeout (60s). A short idle timeout cancels in-flight chunked-response
    // writes under load, truncating the response before the terminal chunk and causing the client to see
    // an EOF mid-read. See https://github.com/http4s/http4s/issues/6427. Teardown is handled by
    // withShutdownTimeout(0) below, so the short timeout is not needed for test speed.
