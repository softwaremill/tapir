...
trait Http4sServerRoutesInterpreter[F[_]] extends Http4sServerInterpreter[F, F] {
...
object Http4sServerRoutesInterpreter {
  def apply[F[_]]()(implicit _fs: Concurrent[F], _fcs: ContextShift[F], _timer: Timer[F]): Http4sServerRoutesInterpreter[F] = {
    new Http4sServerRoutesInterpreter[F] {
  ...
  )(implicit _fs: Concurrent[F], _fcs: ContextShift[F], _timer: Timer[F]): Http4sServerRoutesInterpreter[F] = {
    new Http4sServerRoutesInterpreter[F] {
      override implicit def gs: Sync[F] = _fs
