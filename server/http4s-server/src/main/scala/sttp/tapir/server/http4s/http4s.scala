...
package object http4s extends TapirHttp4sServer {
  def serverSentEventBody[F[_]]: StreamBodyIO[fs2.Stream[F, Byte], fs2.Stream[F, ServerSentEvent], Fs2Streams[F]] = {
    val fs2Streams = Fs2Streams[F]
    streamBody(fs2Streams)(Schema.binary, CodecFormat.TextEventStream(), Some(Charset.forName("UTF-8")))
      .map(Http4sServerInterpreter.parseBytesToSSE(fs2Streams))(Http4sServerInterpreter.serialiseSSEToBytes(fs2Streams))
  }
}
