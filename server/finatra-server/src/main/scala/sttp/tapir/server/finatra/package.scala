    ...
    def toRoute(logic: I => Future[Either[E, O]])(implicit serverOptions: FinatraServerOptions): FinatraRoute = {
      val handler = { request: Request =>
        ...
        def valuesToResponse(values: DecodeInputsResult.Values): Future[Response] = {
          ...
          logic(i)
            .map {
              case Right(result) => OutputToFinatraResponse(Status(ServerDefaults.StatusCodes.error.code), e.output, result)
              case Left(err)     => OutputToFinatraResponse(Status(ServerDefaults.StatusCodes.success.code), e.errorOutput, err)
            }
            .onFailure {
              case NonFatal(ex) =>
                error(ex)
