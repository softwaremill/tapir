...
class SttpRequestBody[F[_]](implicit ME: MonadError[F]) extends RequestBody[F, AnyStreams] {
  ...
  private def body(serverRequest: ServerRequest): Either[Array[Byte], Seq[Part[sttp.client4.GenericRequestBody[_]]]] =
    sttpRequest(serverRequest).body match {
      ...
      case BasicMultipartBody(parts)  => Right(parts)
      case StreamBody(_)              => throw new IllegalArgumentException("Stream body provided while endpoint accepts raw body type")
      case MultipartStreamBody(_)     => throw new IllegalArgumentException("Stream body provided while endpoint accepts raw body type")
    }
