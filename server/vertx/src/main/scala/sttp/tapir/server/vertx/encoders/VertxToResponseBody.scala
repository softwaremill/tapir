  ...
  override def fromRawValue[R](v: R, headers: HasHeaders, format: CodecFormat, bodyType: RawBodyType[R]): RoutingContext => Future[Void] = { rc =>
    ...
    bodyType match {
      ...
      case RawBodyType.FileBody         =>
        ...
        tapirFile.range
          .flatMap(range => range.start.map(start => resp.sendFile(tapirFile.toPath.toString, start, range.contentLength)))
          .getOrElse(resp.sendFile(tapirFile.toFile.toString))
