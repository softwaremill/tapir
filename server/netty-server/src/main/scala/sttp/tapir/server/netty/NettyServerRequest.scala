...
case class NettyServerRequest(req: HttpRequest, attributes: AttributeMap = AttributeMap.Empty, multiPartData: () => List[InterfaceHttpData] = () => Nil) extends ServerRequest {
  ...
  override def withUnderlying(underlying: Any): ServerRequest =
    NettyServerRequest(req = underlying.asInstanceOf[HttpRequest], attributes, multiPartData)
}
