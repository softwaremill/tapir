...
import sttp.tapir.{CodecFormat, FileRange, InputStreamRange, RawBodyType, RawPart}

import java.io.InputStream
import java.nio.ByteBuffer

trait NettyToResponseBodyCommon[S] extends ToResponseBody[NettyResponse, S] {
  ...
  override def fromRawValue[R](v: R, headers: HasHeaders, format: CodecFormat, bodyType: RawBodyType[R]): NettyResponse = {
    bodyType match {
      ...
      case m: RawBodyType.MultipartBody => handleMultipart(m)(v)
    }
  }
  ...
  private def handleMultipart(
      m: RawBodyType.MultipartBody
  )(v: Seq[RawPart]): NettyResponse = {
  ...
  private def rawValueToContentBody[R](bodyType: RawBodyType[R], part: Part[R]): ContentBody = {
    ...
    bodyType match {
      ...
      case RawBodyType.ByteArrayBody =>
        new ByteArrayBody(r, ContentType.create(contentType), partToFileNameUnsafe(part))
      case RawBodyType.ByteBufferBody =>
        val array: Array[Byte] = new Array[Byte](r.remaining)
        r.get(array)
        new ByteArrayBody(array, ContentType.create(contentType), partToFileNameUnsafe(part))
      ...
      case RawBodyType.InputStreamRangeBody =>
        new InputStreamBody(r.inputStream(), ContentType.create(contentType), partToFileNameUnsafe(part))
      case RawBodyType.InputStreamBody =>
        new InputStreamBody(r, ContentType.create(contentType), partToFileNameUnsafe(part))
      case _: RawBodyType.MultipartBody =>
        throw new UnsupportedOperationException("Nested multipart messages are not supported.")
    }
  }

  private def partToFileNameUnsafe[R](part: Part[R]) =
    part.fileName match {
      case Some(filename) => filename
      case None           => throw new IllegalArgumentException("File name is required for multipart body parts")
    }
}
