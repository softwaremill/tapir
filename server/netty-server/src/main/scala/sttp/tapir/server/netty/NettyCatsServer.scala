...
case class NettyCatsServer[F[_]: Async](routes: Vector[Route[F]], options: NettyCatsServerOptions[F]) {
  def addEndpoint(se: ServerEndpoint[_, _, _, _, _, Any, F]): NettyCatsServer[F] = addEndpoints(List(se))
  def addEndpoint(se: ServerEndpoint[_, _, _, _, _, Any, F], overrideOptions: NettyCatsServerOptions[F]): NettyCatsServer[F] =
    addEndpoints(List(se), overrideOptions)
  def addEndpoints(ses: List[ServerEndpoint[_, _, _, _, _, Any, F]]): NettyCatsServer[F] = addRoute(
    NettyCatsServerInterpreter(options).toRoute(ses)
  )
  def addEndpoints(ses: List[ServerEndpoint[_, _, _, _, _, Any, F]], overrideOptions: NettyCatsServerOptions[F]): NettyCatsServer[F] =
    addRoute(
