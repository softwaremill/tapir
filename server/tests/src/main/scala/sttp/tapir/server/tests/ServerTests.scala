...
abstract class ServerTests[F[_], S, R, ROUTE](val streams: Streams[S])
    extends AnyFunSuite
    with Matchers
  ...
  def inputStreamSupport: Boolean = true
  ...
  testServer[String, Nothing, String](in_query_out_infallible_string)((fruit: String) => pureResult(s"fruit: $fruit".asRight[Nothing])) {
    baseUri => basicRequest.get(uri"$baseUri?fruit=kiwi").send(backend).map(_.body shouldBe Right("fruit: kiwi"))
  }
  ...
  if (streams != NoStreams) {
    ...
    // TODO: extract as separate streaming tests to avoid the typecast
    testServer(in_stream_out_stream(streams).asInstanceOf[Endpoint[streams.BinaryStream, Unit, streams.BinaryStream, R]])(
      (s: streams.BinaryStream) => pureResult(s.asRight[Unit])
    ) { baseUri =>
    ...
    testServer(
      in_stream_out_stream_with_content_length(streams)
        .asInstanceOf[Endpoint[(Long, streams.BinaryStream), Unit, (Long, streams.BinaryStream), R]]
  ...
  testServer(
    ...
    basicRequest.get(uri"$baseUri/customer").send(backend).map(response => response.code shouldBe StatusCode.NotFound) >>
      ...
      basicRequest.get(uri"$baseUri/customer/asd/orders/2/xyz").send(backend).map(response => response.code shouldBe StatusCode.NotFound)
  }
  ...
  testServer(Validation.in_valid_json, "support jsonBody validation with wrapped type")((_: ValidFruitAmount) =>
    ...
    basicRequest.get(uri"$baseUri").body("""{"fruit":"orange","amount":11}""").send(backend).map(_.code shouldBe StatusCode.Ok) >>
      basicRequest.get(uri"$baseUri").body("""{"fruit":"orange","amount":0}""").send(backend).map(_.code shouldBe StatusCode.BadRequest) >>
  ...
  val backend: SttpBackend[IO, Any] = AsyncHttpClientCatsBackend[IO]().unsafeRunSync()
