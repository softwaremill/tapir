...
import sttp.tapir.grpc.protobuf.ProtoSchemaGenerator
import sttp.tapir.server.akkagrpc.AkkaGrpcServerInterpreter

object GrpcBooksExample extends App with StrictLogging {
  ...
  object Endpoints {
    ...
    val booksListing: PublicEndpoint[Limit, String, Vector[Book], Any] = baseEndpoint.get
      .in("list" / "all")
      .in(limitParameter)
      .out(grpcBody[Vector[Book]])

    //TODO discuss building message definitions from multiple inputs
  ...
  def renderToFile(): Unit =
    ProtoSchemaGenerator.renderToFile(
      path = "grpc/examples/src/main/protobuf/books_example.proto",
      packageName = "sttp.tapir.grpc.examples.books_example.gen",
      endpoints = List(addBook, booksListing, booksListingByGenre)
    )
  ...
  def startServer(serverEndpoints: List[ServerEndpoint[Any, Future]]): Unit = {
    ...
    implicit val actorSystem: ActorSystem = ActorSystem("GrpcBooksExample", conf)
    import actorSystem.dispatcher
    val routes = AkkaGrpcServerInterpreter().toRoute(serverEndpoints)
...
//    import sttp.tapir.grpc.examples.grpc_books_example.gen.{Library => GenLibrary, LibraryClient => GenLibraryClient, AddBookMsg => GenAddBookMsg}
//
...
//  }
  ...
  startServer(booksServerEndpoints)
