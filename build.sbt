...
lazy val perfTests: ProjectMatrix = (projectMatrix in file("perf-tests"))
  ...
  .dependsOn(core, pekkoHttpServer, http4sServer, nettyServer, nettyServerCats, playServer)
...
lazy val jsoniterScala: ProjectMatrix = (projectMatrix in file("json/jsoniter"))
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      "com.github.plokhotnyuk.jsoniter-scala" %%% "jsoniter-scala-macros" % "2.27.1" % Test,
      scalaTest.value % Test
    )
  )
  .jvmPlatform(scalaVersions = scala2And3Versions)
