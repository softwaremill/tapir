object Versions {
  val http4s = "0.23.18"
  val http4sBlazeServer = "0.23.14"
  val http4sBlazeClient = "0.23.14"
  val catsCore = "2.9.0"
  val catsEffect = "3.4.10"
  val circe = "0.14.3"
  val circeGenericExtras = "0.14.3"
  val circeYaml = "0.14.2"
  val sttp = "3.8.15+2-969772bf+20230511-1148-SNAPSHOT"
  val sttpModel = "1.5.5"
  val sttpShared = "0.0.0+650-201a3968+20230511-1105-SNAPSHOT"
  val sttpApispec = "0.3.2"
  val akkaHttp = "10.2.10"
  val akkaStreams = "2.6.20"
  val pekkoHttp = "0.0.0+4342-c527a7c4-SNAPSHOT"
  val pekkoStreams = "0.0.0+26638-1e514f6a-SNAPSHOT"
  val swaggerUi = "4.18.2"
  val upickle = "3.1.0"
  val playJson = "2.9.4"
  val finatra = "22.12.0"
  val catbird = "21.12.0"
  val json4s = "4.0.6"
  val sprayJson = "1.3.6"
  val scalaCheck = "1.17.0"
  val scalaTest = "3.2.15"
  val scalaTestPlusScalaCheck = "3.2.15.0"
  val refined = "0.10.3"
  val enumeratum = "1.7.2"
  val zio1 = "1.0.18"
  val zio1InteropCats = "13.0.0.2"
  val zio1Json = "0.2.0"
  val zio1InteropReactiveStreams = "1.3.12"
  val zio = "2.0.13"
  val zioInteropCats = "23.0.0.4"
  val zioInteropReactiveStreams = "2.0.1"
  val zioJson = "0.5.0"
  val playClient = "2.1.10"
  val playServer = "2.8.19"
  val tethys = "0.26.0"
  val vertx = "4.4.0"
  val jsScalaJavaTime = "2.5.0"
  val nativeScalaJavaTime = "2.4.0-M3"
  val jwtScala = "9.2.0"
  val derevo = "0.13.0"
  val newtype = "0.4.4"
  val monixNewtype = "0.2.3"
  val zioPrelude = "1.0.0-RC18"
  val awsLambdaInterface = "2.3.2"
  val armeria = "1.23.1"
  val scalaJava8Compat = "1.0.2"
  val scalaCollectionCompat = "2.10.0"
  val fs2 = "3.6.1"
  val decline = "2.4.1"
  val quicklens = "1.9.3"
  val openTelemetry = "1.26.0"
  val mockServer = "5.15.0"
  val dogstatsdClient = "4.2.0"
  val nettyAll = "4.1.92.Final"
}
