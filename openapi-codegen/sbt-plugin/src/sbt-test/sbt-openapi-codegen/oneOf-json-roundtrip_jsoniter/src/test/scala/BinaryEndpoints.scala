...
import sttp.client3.{Response, UriContext, asString, asStringAlways}
import sttp.monad.FutureMonad
import sttp.tapir.generated.TapirGeneratedEndpoints
import sttp.tapir.generated.TapirGeneratedEndpoints.{
  PostCustomContentNegotiationBody0In,
  PostCustomContentNegotiationBody0Out,
  PostCustomContentNegotiationBody1Err,
  PostCustomContentNegotiationBody1In,
  PostCustomContentNegotiationBody1Out,
  PostCustomContentNegotiationBodyIn,
  PostCustomContentNegotiationBodyStringErr
}
  ...
  "binary endpoint negotiation works" in {
    ...
    val route1 =
      TapirGeneratedEndpoints.postCustomContentNegotiation
        .serverLogicSuccess({ case (accept, body) => // source: Source[String, PostCustomContentNegotiationBodyIn] =>
          ...
          def out = accept match {
            case "text/csv" =>
              PostCustomContentNegotiationBody0Out(
                org.apache.pekko.stream.scaladsl.Source[ByteString](immutable.Iterable.fill(100)(produceCsv()))
                  .concat(Source.fromFuture(running).map(_ => ByteString("FIN")))
              )
            case _ =>
              PostCustomContentNegotiationBody1Out(
                org.apache.pekko.stream.scaladsl.Source[ByteString](immutable.Iterable.fill(100)(produceSpreadsheet()))
                  .concat(Source.fromFuture(running).map(_ => ByteString("FIN")))
              )
          }
    ...
    val route2 =
      TapirGeneratedEndpoints.postCustomContentNegotiation
        .serverLogic[Future]({ case (accept, _) =>
          Future.successful(Left(accept match {
            case "text/csv" => PostCustomContentNegotiationBodyStringErr("some csv error")
            case _          => PostCustomContentNegotiationBody1Err("some spreadsheet error".getBytes("utf-8"))
          }))
        })
