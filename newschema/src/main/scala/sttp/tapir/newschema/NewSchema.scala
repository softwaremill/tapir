...
object TestMacro:
  inline def test[T]: T = ${ testImpl[T] }
  def testImpl[T: Type](using Quotes): Expr[T] =
    import quotes.reflect.*
    Expr.summon[T] match
      case None    => report.errorAndAbort(s"Cannot find ${Type.show[T]}!")
      case Some(s) => s

object Derive2:
  inline def derive2[T](using genericDerivationConfig: Configuration): Schema[T] = ${ Derive2.derive2Impl[T]('genericDerivationConfig) }

  def derive2Impl[T: Type](genericDerivationConfig: Expr[Configuration])(using Quotes): Expr[Schema[T]] =
    new Derive2(genericDerivationConfig).derive2Impl(genericDerivationConfig)
...
private class Derive2(genericDerivationConfig: Expr[Configuration])(using Quotes):
  import quotes.reflect.*
  ...
  private def coproductSchemaType[T: Type](tpe: TypeRepr): Expr[SCoproduct[T]] =
    val childNameTagSchemaExprList = tpe.typeSymbol.children.map { childSymbol =>
      println("ZZZ " + childSymbol.tree)
      println(childSymbol.flags.is(Flags.Module))
      println(tpe.memberType(childSymbol))
  ...
  private def withCache[T: Type](typeInfo: TypeInfo, annotations: Annotations)(f: => Expr[Schema[T]]): Expr[Schema[T]] =
    import Derive2.deriveInProgress
