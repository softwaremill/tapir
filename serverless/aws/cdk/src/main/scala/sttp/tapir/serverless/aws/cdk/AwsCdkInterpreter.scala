...
trait AwsCdkInterpreter {
  ...
  def toCdkAppTemplate[F[_]: Sync](se: ServerEndpoint[Any, F]): CdkAppTemplate[F] = new CdkAppTemplate[F](Set(se), awsCdkOptions)

  def toCdkAppTemplate[F[_]: Sync](ses: Iterable[ServerEndpoint[Any, F]]): CdkAppTemplate[F] =
    new CdkAppTemplate[F](ses.toSet, awsCdkOptions)
}
