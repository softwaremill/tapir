...
/** Requires running sam-local process with template generated by `AwsCdkAppTemplate`, it's automated in sbt test task but requires sam cli,
  * cdk and npm installed.
  */
class AwsCdkAppLocalHttpTest extends AnyFunSuite {

  private val baseUri: Uri = uri"http://localhost:3010"
