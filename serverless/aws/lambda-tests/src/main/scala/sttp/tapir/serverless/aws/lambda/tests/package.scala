...
//
//  val in_query_out_string_endpoint: ServerEndpoint[String, Unit, String, Any, IO] =
//    in_query_out_string.serverLogic(fruit => IO.pure(s"fruit: $fruit".asRight[Unit]))
//
...
//  }
  ...
  val in_headers_out_headers_endpoint: ServerEndpoint[List[Header], Unit, List[Header], Any, IO] = in_headers_out_headers.serverLogic { headers =>IO.pure(headers.asRight[Unit]) }
